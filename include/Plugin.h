...
/* describe reason of notification: database is loaded/unloaded, settings are
 * changed */
...
struct PackedDate {
  ...
  // higher 32 bits
  unsigned int Minute
      : 6; // 0..59                       63 is reserved as EOD marker
  unsigned int Hour
      : 5; // 0..23                       31 is reserved as EOD marker
...
struct InfoSite {
  ...
  struct StockInfo *(*AddStockNew)(
      const char
          *pszTicker); // only available if called from AmiBroker 5.27 or higher
};
...
// handles new Quotation format with 64 bit date/time stamp and floating point
// volume/open int and new Aux fields it also takes pointer to context that is
// reserved for future use (can be null) Called by AmiBroker 5.27 and above
...
struct _IntradaySettings {
  ...
  DATE_TIME_INT SessionStart; // bit encoding HHHHH.MMMMMM.0000	 hours << 10 | (
                              // minutes << 4 )
...
struct StockInfo {
  ...
  int DataSource;   /* the ID of the data plug-in, 0 - accept workspace settings
                     */
  int DataLocalMode; /* local mode of operation - 0 - accept workspace settings,
                        1 - store locally, 2 - don't store locally */
  int MarketID;
  int GroupID;
  int IndustryID;
  int GICS;
  ...
  int MoreFlags;       /*  */
  float MarginDeposit; /* new futures fields - active if SI_MOREFLAGS_FUTURES is
                          set */
  float PointValue;
  float RoundLotSize;
  float
      TickSize; /* new futures fields - active if SI_MOREFLAGS_FUTURES is set */
...
// settings are changed, or right mouse button in the plugin status area is
// clicked
...
struct OptimizeParams {
  int Mode; // 0 - gets defaults, 1 - retrieves settings from formula (setup
            // phase), 2 - optimization phase
  int WalkForwardMode; // 0 - none (regular optimization), 1-in-sample, 2 - out
                       // of sample
  int Engine; // optimization engine selected - 0 means - built-in exhaustive
              // search
  int Qty;    // number of variables to optimize
  int LastQty;
  BOOL CanContinue; // boolean flag 1 - means optimization can continue, 0 -
                    // means aborted by pressing "Cancel" in progress dialog or
                    // other error
  BOOL DuplicateCheck; // boolean flag 1 - means that AmiBroker will first check
                       // if same param set wasn't used already and if duplicate
                       // is found it won't run backtest, instead will return
                       // previously stored value
  int Reserved;
  char *InfoText; // pointer to info text buffer (providing text display in the
                  // progress dialog)
  int InfoTextSize; // the size (in bytes) of info text buffer
  __int64 Step;     // current optimization step (used for progress indicator) -
                    // automatically increased with each iteration
  __int64 NumSteps; // total number of optimization steps (used for progress
                    // indicator)
...
PLUGINAPI int OptimizerRun(struct OptimizeParams *pParams,
                           double (*pfEvaluateFunc)(void *), void *pContext);
